...
func IsCRProcessed(ctx context.Context, ctrlClient client.Client, objectMeta v1.Object) (bool, error) {
	...
	if releaseVersionLabel == "" {
		...
		clusterName := objectMeta.GetLabels()[v1alpha3.ClusterLabelName]
		if clusterName == "" {
		...
		// release label
		cluster, err := util.GetClusterByName(ctx, ctrlClient, objectMeta.GetNamespace(), clusterName)
		if err != nil {
